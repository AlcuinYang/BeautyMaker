...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="doubao-seedream-4.0" key="BMB6AP1M"><RenderMd content={`指定生成图像的尺寸信息，支持以下两种方式，不可混用。
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="doubao-seedream-3.0-t2i" key="kghENadO"><RenderMd content={`指定生成图像的宽高像素值。
...
`}></RenderMd></Tabs.TabPane>
<Tabs.TabPane title="doubao-seededit-3.0-i2i" key="MKsftGMr"><RenderMd content={`指定生成图像的宽高像素值。**当前仅支持 adaptive。** 
